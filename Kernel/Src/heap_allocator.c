...
TinyStatus_t kernel_heap_init(void) {
    return memory_allocator_init((uint32_t)&_end, (size_t)&_Min_Heap_Size, MEM_ALIGNMENT, &heap);
}
 ...
 *         satisfied.
 */
void* kernel_malloc(size_t size) { return memory_allocator_alloc(size, &heap); }
 ...
 */
TinyStatus_t kernel_free(void* ptr) { return memory_allocator_free(ptr, &heap); }
